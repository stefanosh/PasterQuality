...
for i, (train_index, test_index) in enumerate(kf.split(paster_ids)):
#     os.makedirs(f'{dir_path}/split_{i}', exist_ok=True)
    os.makedirs(f"{dir_path}/csv/split_{i}/", exist_ok=True)
    os.makedirs(f"{dir_path}/npy/split_{i}/", exist_ok=True)
    ...
    train_set.to_csv(f"{dir_path}/csv/split_{i}/train_set.csv")
    test_set.to_csv(f"{dir_path}/csv/split_{i}/test_set.csv")
    ...
    X_train.to_csv(f"{dir_path}/csv/split_{i}/X_train.csv", index=False)
    X_val.to_csv(f"{dir_path}/csv/split_{i}/X_val.csv", index=False)
    X_test.to_csv(f"{dir_path}/csv/split_{i}/X_test.csv", index=False)
    y_train.to_csv(f"{dir_path}/csv/split_{i}/y_train.csv", index=False)
    y_val.to_csv(f"{dir_path}/csv/split_{i}/y_val.csv", index=False)
    y_test.to_csv(f"{dir_path}/csv/split_{i}/y_test.csv", index=False)
    ...
    for dataset in ["train", "val", "test"]:
        np.save(
            f"{dir_path}/npy/split_{i}/N_{dataset}.npy",
            eval(f"X_{dataset}"),
        )
        np.save(
             f"{dir_path}/npy/split_{i}/y_{dataset}.npy",
            eval(f"y_{dataset}"),
        )

    # Save the required metadata as JSON
    dataset_name = f"split_{i}"
    json_data = {}
    file_path = f"{dir_path}/npy/split_{i}/info.json"
